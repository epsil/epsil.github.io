/**
 * Better footnotes.
 *
 * TODO:
 *
 * - Smart detection of parent elements (header, table, ...)
 * - Smart insertion (paragraph: before, header: after)
 * - Reusable functions (e.g., of two args: mark and def)
 * - Handle nested footnotes
 */

import $ from 'jquery';

const headingSelector = 'h1, h2, h3, h4, h5, h6';
const listSelector = 'ul, ol, dl';
const tableSelector = 'table';

export function fixFootnotes() {
  return this.each(function() {
    const body = $(this);
    body.find('.footnote-ref a').each(function() {
      const a = $(this);
      let html = a.html();
      html = html
        .replace('[', '<span class="left-bracket">[</span>')
        .replace(']', '<span class="right-bracket">]</span>')
        .replace(
          /(:)([0-9]+)/,
          '<span class="suffix-colon">$1</span><span class="suffix-number">$2</span>'
        );
      a.html(html);
      const sup = a.parent();
      const p = sup.parent();
      const id = a.attr('href').replace(/:.*/, '');
      a.attr('href', id);
      const note = body.find(id);
      const backref = note.find('.footnote-backref');
      const text = note
        .text()
        .trim()
        .replace(/(\s*\u21a9.*\s*)+$/, '');
      const source = p.text().trim();
      a.attr('title', text);
      backref.attr('title', source);
    });
  });
}

// marginalia
export function addSidenotes() {
  return this.each(function() {
    const body = $(this);
    body.find('.footnote-ref a').each(function() {
      const a = $(this);
      const sup = a.parent();
      let parent = sup.parent();
      if (sup.parents(tableSelector).length) {
        parent = sup.parents(tableSelector).last();
      } else if (sup.parents(headingSelector).length) {
        parent = sup.parents(headingSelector).last();
      } else if (sup.parents(listSelector).length) {
        parent = sup.parents(listSelector).last();
      }
      const idSelector = a.attr('href');
      const id = idSelector.replace(/^#/, '');
      const note = body.find(idSelector);
      const backref = note.find('.footnote-backref');
      const backrefLi = backref.parent().parent();
      const num = a
        .clone()
        .attr('href', backref.attr('href'))
        .attr('title', backref.attr('title'))
        .attr('id', id.replace(/^fn/, 'sidenote'))
        .wrap('<sup class="ref-mark">')
        .parent();
      const sidenote = $('<aside class="sidenote">').html(backrefLi.html());
      sidenote
        .children()
        .first()
        .prepend(num);
      sidenote.prepend('<hr class="sidenote-ruler">');
      if (parent.is('h1, h2, h3, h4, h5, h6')) {
        sidenote.insertAfter(parent);
      } else {
        sidenote.insertBefore(parent);
      }
    });
    body.find('.footnotes-list, .footnotes-sep').addClass('endnotes');
  });
}

// the following is cribbed from http://gwern.net/
let footnotetimeout = false;

export function setup() {
  const footnotelinks = $('.footnote-ref a');
  footnotelinks.off('mouseover', footnoteover);
  footnotelinks.off('mouseout', footnoteoout);
  footnotelinks.on('mouseover', footnoteover);
  footnotelinks.on('mouseout', footnoteoout);
}

export function addFootnoteHandlers() {
  return this.each(function() {
    const body = $(this);
    body.find('.footnote-ref a').each(function() {
      const a = $(this);
      a.removeAttr('title');
      a.off('mouseover', footnoteover);
      a.off('mouseout', footnoteoout);
      a.on('mouseover', footnoteover);
      a.on('mouseout', footnoteoout);
    });
  });
}

export function footnoteover() {
  clearTimeout(footnotetimeout);
  $('#footnotediv').stop();
  $('#footnotediv').remove();

  const id = $(this)
    .attr('href')
    .substr(1);
  const position = $(this).offset();

  const div = $(document.createElement('div'));
  div.attr('id', 'footnotediv');
  div.bind('mouseover', divover);
  div.bind('mouseout', footnoteoout);

  const el = document.getElementById(id);
  div.html('<div>' + $(el).html() + '</div>');

  // const body = $('.e-content') || $(document.body)
  const body = $(document.body);
  body.append(div);

  let left = position.left;
  if (left + 420 > $(window).width() + $(window).scrollLeft()) {
    left = $(window).width() - 420 + $(window).scrollLeft();
  }
  let top = position.top + 20;
  if (top + div.height() > $(window).height() + $(window).scrollTop()) {
    top = position.top - div.height() - 15;
  }
  div.css({
    left: left,
    top: top,
    opacity: 1,
    position: 'absolute'
  });
}

export function footnoteoout() {
  footnotetimeout = setTimeout(function() {
    $('#footnotediv').animate(
      {
        opacity: 0
      },
      600,
      function() {
        $('#footnotediv').remove();
      }
    );
  }, 100);
}

export function divover() {
  clearTimeout(footnotetimeout);
  $('#footnotediv').stop();
  $('#footnotediv').css({
    opacity: 1
  });
}

if ($ && $.fn) {
  $.fn.fixFootnotes = fixFootnotes;
  $.fn.addSidenotes = addSidenotes;
  $.fn.addFootnoteHandlers = addFootnoteHandlers;
}

// $(document).ready(function() {
//   setup()
// })

export default {
  fixFootnotes,
  addSidenotes,
  setup,
  addFootnoteHandlers,
  footnoteover,
  footnoteoout,
  divover
};
